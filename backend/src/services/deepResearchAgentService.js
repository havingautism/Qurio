...
const buildStepPrompt = ({ planMeta, step, stepIndex, priorFindings, sourcesList }) => {
...
const buildFinalReportPrompt = ({ planMeta, question, findings, sourcesList }) => {
  return `You are a deep research writer producing a final report.

Question: ${question || planMeta.goal || 'N/A'}
        ...
        if (!isLocalToolName(toolName)) {
          ...
          toolEvents.push(
            buildToolResultEvent(
              ...
              Date.now() - startedAt,
              undefined,
              { step: typeof stepIndex === 'number' ? stepIndex + 1 : undefined, total: totalSteps },
            ),
          )
          continue
        }
...
export const streamDeepResearch = async function* (params) {
  ...
  const combinedTools = [...(Array.isArray(tools) ? tools : []), ...agentToolDefinitions].filter(Boolean)
  ...
  const planContent =
    typeof plan === 'string' && plan.trim().length
      ? plan
      : await generateResearchPlan(provider, question || '', apiKey, baseUrl, model)
