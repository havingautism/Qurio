...
static ADAPTER_CACHE: Lazy<HashMap<String, Arc<dyn ProviderAdapter>>> = Lazy::new(|| {
    ...
    map.insert("openai".to_string(), Arc::new(OpenAIAdapter::new()) as Arc<dyn ProviderAdapter>);
    map.insert("openai_compatibility".to_string(), Arc::new(OpenAICompatibilityAdapter::new()) as Arc<dyn ProviderAdapter>);

    // SiliconFlow
    map.insert("siliconflow".to_string(), Arc::new(SiliconFlowAdapter::new()) as Arc<dyn ProviderAdapter>);

    // GLM (Zhipu AI)
    map.insert("glm".to_string(), Arc::new(GLMAdapter::new()) as Arc<dyn ProviderAdapter>);

    // Kimi (Moonshot AI)
    map.insert("kimi".to_string(), Arc::new(KimiAdapter::new()) as Arc<dyn ProviderAdapter>);

    // ModelScope
    map.insert("modelscope".to_string(), Arc::new(ModelScopeAdapter::new()) as Arc<dyn ProviderAdapter>);

    // Gemini (Google)
    map.insert("gemini".to_string(), Arc::new(GeminiAdapter::new()) as Arc<dyn ProviderAdapter>);

    // Nvidia NIM
    map.insert("nvidia".to_string(), Arc::new(NvidiaNimAdapter::new()) as Arc<dyn ProviderAdapter>);

    // MiniMax
    map.insert("minimax".to_string(), Arc::new(MinimaxAdapter::new()) as Arc<dyn ProviderAdapter>);
