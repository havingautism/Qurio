...
pub fn get_base_url(provider: &str) -> Option<&'static str> {
    let canonical = provider_alias(provider);
    PROVIDER_BASE_URLS
        .iter()
...
pub fn supports_capability(provider: &str, capability: &str) -> bool {
    match capability {
        "streaming" => get_capabilities(provider)
            .map(|c| c.supports_streaming)
            .unwrap_or(false),
        "tool_calls" => get_capabilities(provider)
            .map(|c| c.supports_tool_calls)
            .unwrap_or(false),
        "streaming_tool_calls" => get_capabilities(provider)
            .map(|c| c.supports_streaming_tool_calls)
            .unwrap_or(false),
        "json_schema" => get_capabilities(provider)
            .map(|c| c.supports_json_schema)
            .unwrap_or(false),
        "thinking" => get_capabilities(provider)
            .map(|c| c.supports_thinking)
            .unwrap_or(false),
        "vision" => get_capabilities(provider)
