// GLM Custom Provider Module
//
// This module implements a custom provider for GLM (智谱AI) that properly
...
impl GLMClient {
    pub fn builder() -> GLMClientBuilder {
        GLMClientBuilder {
            api_key: None,
            base_url: None,
        }
    ...
    pub fn agent(self, model: String) -> rig::agent::AgentBuilder<GLMCompletionModel> {
        rig::agent::AgentBuilder::new(GLMCompletionModel {
            client: self,
            model,
        })
    }
...
impl GLMClientBuilder {
    ...
    pub fn build(self) -> Result<GLMClient, String> {
        let api_key = self.api_key.ok_or("API key is required")?;
        let base_url = self.base_url.unwrap_or_else(|| "https://open.bigmodel.cn/api/paas/v4".to_string());
        
        Ok(GLMClient::new(api_key, base_url))
    }
}

// Implement CompletionClient trait for GLMClient
impl CompletionClient for GLMClient {
    type CompletionModel = GLMCompletionModel;
...
/// GLM Completion Model
#[derive(Clone, Debug)]
pub struct GLMCompletionModel {
    client: GLMClient,
    model: String,
}
...
#[derive(Debug, Clone)]
struct GLMToolCallState {
    id: String,
    name: String,
    arguments: String,
}

/// GLM Streaming Delta - includes reasoning_content field
#[derive(Debug, Deserialize)]
pub struct GLMStreamingDelta {
    #[serde(default)]
    pub content: Option<String>,
    
    #[serde(default)]
    pub reasoning_content: Option<String>,  // ← Key field!
    
    #[serde(default)]
    pub tool_calls: Vec<GLMToolCall>,
}
...
#[derive(Debug, Deserialize)]
pub struct GLMStreamingChoice {
    pub delta: GLMStreamingDelta,
    pub finish_reason: Option<String>,
}

#[derive(Debug, Deserialize)]
pub struct GLMStreamingChunk {
    pub choices: Vec<GLMStreamingChoice>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GLMStreamingResponse {
    pub content: String,
}

// Implement GetTokenUsage trait
impl GetTokenUsage for GLMStreamingResponse {
    fn token_usage(&self) -> Option<rig::completion::Usage> {
        // TODO: Extract actual usage from GLM response
        None
    }
}
...
impl rig::completion::CompletionModel for GLMCompletionModel {
    type Response = GLMStreamingResponse;
    type StreamingResponse = GLMStreamingResponse;
    type Client = GLMClient;
    ...
    async fn completion(
        ...
        Err(CompletionError::ProviderError(
            "Non-streaming not implemented for GLM custom provider yet".to_string(),
        ))
    }
    ...
    async fn stream(
        ...
        stream_glm_completion(&self.client, &self.model, request).await
    }
}



async fn stream_glm_completion(
    client: &GLMClient,
    model: &str,
    request: CompletionRequest,
) -> Result<StreamingCompletionResponse<GLMStreamingResponse>, CompletionError> {
    ...
    for msg in request.chat_history.iter() {
        messages.push(convert_message_to_glm(msg)?);
    }
    ...
    let stream = stream! {
        ...
        let mut tool_calls: HashMap<usize, GLMToolCallState> = HashMap::new();
                    ...
                    while let Some(line_end) = lines_buffer.find('\n') {
                        ...
                        if let Some(data) = line.strip_prefix("data: ") {
                            ...
                            match serde_json::from_str::<GLMStreamingChunk>(data) {
                                    ...
                                    if let Some(choice) = glm_chunk.choices.first() {
                                        ...
                                        if !delta.tool_calls.is_empty() {
                                            ...
                                            for tool_call in &delta.tool_calls {
                                                ...
                                                let existing_tool_call = tool_calls.entry(index).or_insert_with(|| GLMToolCallState {
                                                    id: String::new(),
                                                    name: String::new(),
                                ...
                                Err(e) => {
                                    eprintln!("[GLM] Failed to parse chunk: {} - Data: {}", e, data);
                                }
                            }
                        }
                    }
        ...
        // Final response
        yield Ok(RawStreamingChoice::FinalResponse(GLMStreamingResponse {
            content: String::new(),
        }));
    };
...
// Helper function to convert rig messages to GLM format
fn convert_message_to_glm(msg: &rig::completion::Message) -> Result<Value, CompletionError> {
