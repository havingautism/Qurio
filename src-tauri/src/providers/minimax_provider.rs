// MiniMax Custom Provider Module
//
// This module implements a custom provider for MiniMax AI (OpenAI-compatible)
// that properly handles streaming responses with reasoning_details support.
//
// Key features:
// - OpenAI-compatible API format
// - Streaming support with reasoning_details (MiniMax-specific)
// - Tool calls support
// - Thinking mode via reasoning_split parameter
//
// Based on Rig's official custom provider guide and NVIDIA provider implementation.
...
impl MinimaxClient {
    pub fn builder() -> MinimaxClientBuilder {
        MinimaxClientBuilder {
            api_key: None,
            base_url: None,
        }
    }
    ...
    pub fn agent(self, model: String) -> rig::agent::AgentBuilder<MinimaxCompletionModel> {
        rig::agent::AgentBuilder::new(MinimaxCompletionModel {
            client: self,
            model,
        })
    }
...
impl MinimaxClientBuilder {
    ...
    pub fn build(self) -> Result<MinimaxClient, String> {
        let api_key = self.api_key.ok_or("API key is required")?;
        let base_url = self.base_url.unwrap_or_else(|| "https://api.minimax.io/v1".to_string());

        Ok(MinimaxClient::new(api_key, base_url))
    }
}

// Implement CompletionClient trait for MinimaxClient
impl CompletionClient for MinimaxClient {
    type CompletionModel = MinimaxCompletionModel;
...
#[derive(Debug, Clone)]
struct MinimaxToolCallState {
    id: String,
    name: String,
    arguments: String,
}

/// MiniMax Streaming Delta
...
pub struct MinimaxStreamingDelta {
    ...
    // MiniMax-specific: reasoning content in reasoning_details field
...
pub struct MinimaxStreamingChoice {
    pub delta: MinimaxStreamingDelta,
    #[serde(default)]
...
pub struct MinimaxStreamingChunk {
    pub choices: Vec<MinimaxStreamingChoice>,
    #[serde(default)]
...
pub struct MinimaxFinalResponse {
    pub usage: MinimaxUsage,
}

impl GetTokenUsage for MinimaxFinalResponse {
...
impl rig::completion::CompletionModel for MinimaxCompletionModel {
    type Response = MinimaxFinalResponse;
    type StreamingResponse = MinimaxFinalResponse;
    type Client = MinimaxClient;
    ...
    async fn completion(
        ...
        Err(CompletionError::ProviderError(
            "Non-streaming not implemented for MiniMax custom provider yet".to_string(),
        ))
    }
    ...
    async fn stream(
        ...
        stream_minimax_completion(&self.client, &self.model, request).await
    }
}

async fn stream_minimax_completion(
    client: &MinimaxClient,
    model: &str,
    request: CompletionRequest,
) -> Result<StreamingCompletionResponse<MinimaxFinalResponse>, CompletionError> {
    ...
    // MiniMax thinking mode: set reasoning_split=true in extra_body
    if let Some(additional) = &request.additional_params {
        if let Value::Object(map) = additional {
            // Check if thinking mode is enabled
            let has_thinking = map.values().any(|v| {
            ...
            if has_thinking {
                // MiniMax-specific: use reasoning_split parameter
                request_body["extra_body"] = json!({
                    "reasoning_split": true
                });
            }

            // Add other additional params
            for (key, value) in map.iter() {
                if key != "thinking" && !request_body.get(key).is_some() {
    ...
    let stream = stream! {
        ...
        let mut tool_calls: HashMap<usize, MinimaxToolCallState> = HashMap::new();
        let mut text_content = String::new();
        let mut final_usage: Option<MinimaxUsage> = None;
        ...
        while let Some(chunk_result) = futures::StreamExt::next(&mut stream).await {
            match chunk_result {
                Ok(chunk) => {
                    ...
                    while let Some(line_end) = lines_buffer.find('\n') {
                        ...
                        if let Some(data) = line.strip_prefix("data: ") {
                            ...
                            // Parse and handle reasoning_details from raw JSON
                            // MiniMax uses "reasoning_details" field for thinking content
                            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(data) {
                                if let Some(choices) = parsed.get("choices").and_then(|c| c.as_array()) {
                                    if let Some(first_choice) = choices.first() {
                                        if let Some(delta) = first_choice.get("delta") {
                                            // Handle reasoning_details (MiniMax-specific)
                                            if let Some(reasoning_details) = delta.get("reasoning_details") {
                                                if let Some(reasoning_str) = reasoning_details.as_str() {
                            ...
                            match serde_json::from_str::<MinimaxStreamingChunk>(data) {
                                Ok(minimax_chunk) => {
                                    // Handle usage if present
                                    if let Some(usage) = minimax_chunk.usage {
                                        final_usage = Some(usage);
                                    }

                                    if let Some(choice) = minimax_chunk.choices.first() {
                                        let delta = &choice.delta;
                                        let finish_reason = &choice.finish_reason;

                                        // Handle reasoning_details from structured delta
                                        ...
                                        if let Some(ref tool_calls_vec) = delta.tool_calls {
                                            for tool_call in tool_calls_vec {
                                                ...
                                                let existing_tool_call = tool_calls
                                                    .entry(index)
                                                    .or_insert_with(|| MinimaxToolCallState {
                                                        id: String::new(),
                                                        name: String::new(),
                                ...
                                Err(e) => {
                                    eprintln!("[MiniMax] Failed to parse chunk: {}", e);
                                    continue;
                                }
                            }
                        }
                ...
                Err(e) => {
                    eprintln!("[MiniMax] Stream error: {}", e);
        ...
        let usage = final_usage.unwrap_or_else(|| MinimaxUsage {
            prompt_tokens: None,
            completion_tokens: None,
            total_tokens: None,
        });

        yield Ok(RawStreamingChoice::FinalResponse(MinimaxFinalResponse { usage }));
    };
