...
use rig::{
    ...
    providers::{gemini, moonshot, openai},
    streaming::{StreamedAssistantContent, StreamedUserContent, StreamingChat},
    tool::Tool,
    OneOrMany,
};

// Import our custom GLM provider
...
use crate::modules::research_plan::{ResearchPlanRequest, ResearchPlanResponse, RESEARCH_PLAN_PROMPT_ACADEMIC, RESEARCH_PLAN_PROMPT_GENERAL, RESEARCH_PLAN_SERVICE};
...
#[derive(Clone)]
pub struct TavilyAcademicSearchTool {
    api_key: String,
    http: reqwest::Client,
}
...
async fn rig_complete(
    ...
    let model = payload.model.clone().unwrap_or_else(|| {
        if normalized_provider == "gemini" {
            ...
            get_default_model(normalized_provider)
                .unwrap_or("gpt-4o-mini")
                .to_string()
        }
    });

    let response = match normalized_provider {
        "gemini" => {
        ...
        "moonshot" => {
            let mut builder =
                moonshot::Client::<reqwest::Client>::builder().api_key(payload.api_key.clone());
        ...
        _ => {
            let mut builder = openai::CompletionsClient::<reqwest::Client>::builder()
                .api_key(payload.api_key.clone());
            if let Some(base_url) = resolve_base_url(provider, payload.base_url.as_deref()) {
                builder = builder.base_url(&base_url);
            }
            let client = builder
                .build()
...
async fn stream_chat(
    ...
    let enable_tag_parsing = normalized_provider != "siliconflow";
    let http = state.http.clone();

    let event_stream = match normalized_provider {
        "gemini" => {
            let client = gemini::Client::builder()
                .api_key(payload.api_key.clone())
                .build()
...
Return JSON with keys "title" and "emojis". "emojis" must be an array with 1 emoji character."#;
    ...
    let response_text = match normalized_provider {
        "gemini" => {
            let client = gemini::Client::builder()
                .api_key(payload.api_key.clone())
                .build()
...
Return the result as JSON with key "agentName" (agent name only, or null if no match)."#,
    ...
    let user_content = format!(
        "{}\n\nAvailable agents in {}:\n{}",
        payload.message, space_label, agents_text
    );
...
async fn generate_research_plan(
    ...
    // Use the ResearchPlanService from the new module
    let plan = RESEARCH_PLAN_SERVICE
        .generate(&payload)
        .await
...
async fn generate_related_questions(
    ...
    let provider_lower = normalized_provider.to_lowercase();
    let response_text = match provider_lower.as_str() {
        ...
        .map_err(|err| {
          ...
          (
            ...
            agent.prompt(&prompt_text).await.map_err(|e| {
        ...
        "moonshot" => {
            ...
            let client = builder.build().map_err(|err| {
                eprintln!("Moonshot client build error: {}", err);
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(json!({"error": "Internal server error", "message": format!("{}", err)})),
                )
            ...
            // Set JSON response format for Moonshot provider
            ...
            agent.prompt(&prompt_text).await.map_err(|e| {
                eprintln!("Moonshot prompt error: {}", e);
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(json!({"error": "Internal server error", "message": format!("{}", e)})),
                )
...
) -> Result<(Option<String>, Vec<Message>), (StatusCode, Json<Value>)> {
    ...
    for message in messages {
        match message.role.as_str() {
            ...
            "tool" => {
                ...
                let content_value =
                    extract_text_content(message.content.as_ref()).unwrap_or_default();
...
fn split_prompt_history(
    messages: &mut Vec<Message>,
) -> Result<(Message, Vec<Message>), (StatusCode, Json<Value>)> {
...
fn extract_image_url(value: &Value) -> Option<String> {
    ...
    let Value::Object(image_map) = image else {
        return None;
    };
...
fn convert_tool_call(
    ...
    index: usize,
) -> Option<rig::completion::message::ToolCall> {
...
fn tool_result_content_to_value(
    content: &OneOrMany<rig::completion::message::ToolResultContent>,
) -> Value {
...
fn collect_tavily_sources(value: &Value, sources: &mut HashMap<String, SourceItem>) {
    ...
    let Some(Value::Array(results)) = map.get("results") else {
        return;
    };
    for result in results {
        let Value::Object(item) = result else {
            continue;
        };
        let Some(Value::String(url)) = item.get("url") else {
            continue;
        };
...
pub enum AgentBuilderWrapper<M>
where
...
/// Build a tool-enabled agent for deep research
...
pub fn build_research_agent(
    ...
    tavily_api_key: Option<String>,
    temperature: Option<f64>,
) -> Result<Agent<rig::providers::openai::CompletionModel>, String> {
    // Start with plain agent builder wrapped in our enum
    let mut agent = AgentBuilderWrapper::Plain(client.agent(model));

    // Add Tavily search tool based on research_type
    if let Some(key) = tavily_api_key {
        let http = reqwest::Client::new();
        // Branch on type since we can't use Box<dyn Tool>
        if research_type == "academic" {
            tracing::info!("[DeepResearch] Adding Tavily_academic_search tool for academic research");
            agent = agent.tool(TavilyAcademicSearchTool::new(key, http));
        } else {
            tracing::info!("[DeepResearch] Adding Tavily_web_search tool for general research");
            agent = agent.tool(TavilyWebSearchTool::new(key, http));
        }
    } else {
        tracing::info!("[DeepResearch] No Tavily API key provided, building agent without tools");
    }
    ...
    tracing::info!("[DeepResearch] Agent built successfully for model: {}", model);
    Ok(built_agent)
}
