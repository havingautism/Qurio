...
const SettingsModal = ({ isOpen, onClose, onOpenDatabaseSetup }) => {
  ...
  const [ocrModelSource, setOcrModelSource] = useState('list')
  const [ocrCustomModel, setOcrCustomModel] = useState('')
  const [enablePdfOcr, setEnablePdfOcr] = useState(false)
  const [ocrGroupedModels, setOcrGroupedModels] = useState({})
  const [ocrAvailableProviders, setOcrAvailableProviders] = useState([])
  const [ocrModelsLoading, setOcrModelsLoading] = useState(false)
  const [ocrModelsError, setOcrModelsError] = useState('')
  ...
  useEffect(() => {
    const handleClickOutside = event => {
      ...
      if (
        ocrProviderDropdownRef.current &&
        !ocrProviderDropdownRef.current.contains(event.target)
      ) {
        setIsOcrProviderDropdownOpen(false)
      }
      return
    }
  ...
  const ocrModelCount = useMemo(
    () => Object.values(ocrGroupedModels).reduce((sum, models) => sum + models.length, 0),
    [ocrGroupedModels],
  )
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      if (settings.ocrProvider) setOcrProvider(settings.ocrProvider)
      if (settings.ocrModelSource) setOcrModelSource(settings.ocrModelSource || 'list')
      if (settings.ocrModel) setOcrModel(settings.ocrModel)
      if (settings.ocrModelSource === 'custom') setOcrCustomModel(settings.ocrModel || '')
      ...
      if (settings.databaseProvider) {
        fetchRemoteSettings().then(({ data }) => {
          ...
          if (data.SiliconFlowKey) setSiliconFlowKey(data.SiliconFlowKey)
            ...
            if (data.ocrModelSource) setOcrModelSource(data.ocrModelSource || 'list')
            if (data.ocrModel) setOcrModel(data.ocrModel)
            if (data.ocrModelSource === 'custom') setOcrCustomModel(data.ocrModel || '')
  ...
  const loadOcrModels = async () => {
    ...
    for (const key of PROVIDER_KEYS) {
      ...
      try {
        ...
        const filtered = (Array.isArray(models) ? models : []).filter(matchesOcrKeyword)
    ...
    setOcrAvailableProviders(uniqueProviders)
    setOcrGroupedModels(grouped)
    ...
    const fallbackProvider = uniqueProviders.includes(ocrProvider)
      ? ocrProvider
      : uniqueProviders.includes(apiProvider)
        ? apiProvider
        : uniqueProviders[0]

    if (fallbackProvider && fallbackProvider !== ocrProvider) {
      setOcrProvider(fallbackProvider)
    }
  ...
  useEffect(() => {
    ...
    if (ocrModel && !activeModels.some(model => model.value === ocrModel)) {
      setOcrModel('')
    }
            ...
            {activeTab === 'model' && (
              <div className="flex flex-col gap-8">
                ...
                <div className="flex gap-3 rounded-lg bg-blue-50 p-4 text-sm text-blue-700 dark:bg-blue-900/10 dark:text-blue-300">
                  ...
                  <div>
                    <p className="font-medium">{t('settings.ocrConfiguration')}</p>
                    <p className="opacity-90">{t('settings.ocrConfigurationHint')}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50/50 p-3 dark:border-zinc-800 dark:bg-zinc-800/50">
                  ...
                  <Switch
                    ...
                    title={!canEnablePdfOcr ? t('settings.ocrModelRequired') : ''}
                ...
                ) : ocrAvailableProviders.length === 0 ? (
                  <div className="rounded-lg border border-dashed border-gray-200 p-6 text-center text-sm text-gray-500 dark:border-zinc-700 dark:text-gray-400">
                    <p className="font-medium text-gray-700 dark:text-gray-300">
                      {t('settings.ocrNoProvidersTitle')}
                    </p>
                    <p className="mt-1">{t('settings.ocrNoProvidersHint')}</p>
                  </div>
                ) : (
                  <>
                          ...
                          <div className="flex w-full flex-wrap items-center gap-3">
                            ...
                            <div className="flex rounded-lg border border-gray-200 bg-gray-100 p-0.5 dark:border-zinc-700 dark:bg-zinc-800">
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  setOcrModelSource('custom')
                                  const nextValue = ocrCustomModel || ocrModel || ''
                                  setOcrCustomModel(nextValue)
                                  setOcrModel(nextValue)
                                }}
                                className={clsx(
                                  'rounded-md px-3 py-1 text-xs font-medium transition-all',
                          ...
                          <div className="relative flex flex-col gap-2">
                            ...
                            <Select value={ocrProvider} onValueChange={setOcrProvider}>
                              <SelectTrigger className="h-10 w-full">
                                <SelectValue>
                                  <div className="flex items-center gap-3">
                                    {renderProviderIcon(ocrProvider, {
                                      size: 16,
                                      alt: ocrProviderLabel,
                                    })}
                                    <span>{ocrProviderLabel}</span>
                                  </div>
                                </SelectValue>
                              </SelectTrigger>
                              <SelectContent>
                                {ocrAvailableProviders.map(key => (
                                  <SelectItem key={key} value={key}>
                                    <div className="flex items-center gap-3">
                                      {renderProviderIcon(key, {
                                        size: 16,
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            {ocrModelSource === 'list' ? (
                              <Select
                                ...
                                disabled={!activeOcrModels.length}
                              >
                                <SelectTrigger className="h-10 w-full">
                                  <SelectValue placeholder={t('agents.model.notSelected')}>
                                    <div className="flex items-center gap-2 truncate">
                                      {getModelIcon(ocrModel) && (
                                        <img
                                          src={getModelIcon(ocrModel)}
                                          alt=""
                                          className={clsx(
                                            'h-4 w-4 shrink-0',
                                            getModelIconClassName(ocrModel),
                                          )}
                                        />
                                      )}
                                      <span className="truncate">
