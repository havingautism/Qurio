...
export const buildDocumentQueryPrompt = ({ question, historyForSend, documents }) => {
  ...
  return [
    ...
    `If no document retrieval is needed, return an empty string.`,
    `Bad example: Java代码 编码 注意事项 最佳实践 规范`,
    `Good example: Java代码编写时的注意事项和编码规范`,
    `Bad example: JVM Java虚拟机`,
    `Good example: 什么是JVM Java虚拟机的定义和作用`,
    `Return only the query itself as plain text.`,
    '',
    `User question:\n${question}`,
    historySection,
    docSection,
